...
- **MTP speculative decoding scaffolding** — Gemma4 assistant drafter loader, projection helpers, model-aware verifier result validation, acceptance accounting consistency checks, and staged KV commit/rollback (end-to-end verifier/drafter generation wiring pending)
...
- Transitional `model` helpers validate MTP token/KV keep counts, model-aware MTP verifier logits/activation dimensions, MTP acceptance consistency before KV commit, MTP drafter projection sizing, MoE edge cases, embedding/LM-head/per-layer input backing data, chunked LM-head and batched-prefill dimensions, CPU forward-layer entrypoints, model-specific KV width overflow, and low-level GEMV/GQA product arithmetic; loader/prefill/GPU placement diagnostics are quiet unless `GO_PHERENCE_LOAD_DEBUG` or `GO_PHERENCE_PREFILL_DEBUG` is set.
