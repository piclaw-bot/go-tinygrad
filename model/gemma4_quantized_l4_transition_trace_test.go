...
func TestGemma4QuantizedLayer4TransitionTrace(t *testing.T) {
	if os.Getenv("GEMMA4_TRACE_TEST") == "" {
		t.Skip("set GEMMA4_TRACE_TEST=1 for Gemma4 quantized layer4 transition trace")
	}
	dir := gemma4Path()
	...
	maxAbs, meanAbs := diffStats(cpuFinal[4], cpuOps[opTraceKey{5, "hidden_in"}])
	t.Logf("CPU L4 final_hidden vs L5 hidden_in: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
	maxAbs, meanAbs = diffStats(gpuFinal[4], gpuOps[opTraceKey{5, "hidden_in"}])
	t.Logf("GPU L4 final_hidden vs L5 hidden_in: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
}
