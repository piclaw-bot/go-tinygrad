...
func TestGemma4QuantizedLayer5TransitionTrace(t *testing.T) {
	if os.Getenv("GEMMA4_TRACE_TEST") == "" {
		t.Skip("set GEMMA4_TRACE_TEST=1 for Gemma4 quantized layer5 transition trace")
	}
	dir := gemma4Path()
	...
	maxAbs, meanAbs := diffStats(cpuFinal[5], cpuOps[opTraceKey{6, "hidden_in"}])
	t.Logf("CPU L5 final_hidden vs L6 hidden_in: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
	maxAbs, meanAbs = diffStats(gpuFinal[5], gpuOps[opTraceKey{6, "hidden_in"}])
	t.Logf("GPU L5 final_hidden vs L6 hidden_in: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
}
