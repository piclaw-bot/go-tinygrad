...
func TestGemma4QuantizedLayer7PostAttnSensitivity(t *testing.T) {
	if os.Getenv("GEMMA4_TRACE_TEST") == "" {
		t.Skip("set GEMMA4_TRACE_TEST=1 for Gemma4 quantized layer7 post-attn sensitivity")
	}
	dir := gemma4Path()
	...
	gpuHiddenIn := gpuOps[opTraceKey{7, "hidden_in"}]
	gpuO := gpuOps[opTraceKey{7, "o"}]
	gpuMLPIn := gpuOps[opTraceKey{7, "mlp_input"}]
	cpuMLPIn := cpuOps[opTraceKey{7, "mlp_input"}]
	if len(gpuHiddenIn) == 0 || len(gpuO) == 0 || len(gpuMLPIn) == 0 || len(cpuMLPIn) == 0 {
		t.Fatal("missing layer7 hidden_in/o/mlp_input traces")
	}

	cpuONorm := append([]float32(nil), gpuO...)
	rmsNormInPlace(cpuONorm, m.Layers[7].PostNorm.Data(), float32(m.Config.RMSNormEps))
	...
	rmsNormInPlace(cpuRecon, m.Layers[7].PreFFNNorm.Data(), float32(m.Config.RMSNormEps))
	bf16Slice(cpuRecon)
	maxAbs, meanAbs := diffStats(gpuMLPIn, cpuRecon)
	t.Logf("L7 cpu(preffn(residual+postnorm(gpuO))) vs gpu mlp_input: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
	maxAbs, meanAbs = diffStats(cpuMLPIn, cpuRecon)
	t.Logf("L7 cpu(preffn(residual+postnorm(gpuO))) vs cpu mlp_input: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
	...
	gpu.DevRMSNorm(tmpBuf, oBuf, g.Layers[7].PostNorm, float32(m.Config.RMSNormEps))
	gpu.DevAdd(outBuf, hidBuf, tmpBuf)
	gpu.DevRMSNorm(outBuf, outBuf, g.Layers[7].PreFFNNorm, float32(m.Config.RMSNormEps))
	...
	t.Logf("L7 fresh gpu(preffn(residual+postnorm(gpuO))) vs gpu mlp_input: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
}
