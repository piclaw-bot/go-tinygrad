...
func TestGemma4QuantizedLayer2TransitionTrace(t *testing.T) {
	if os.Getenv("GEMMA4_TRACE_TEST") == "" {
		t.Skip("set GEMMA4_TRACE_TEST=1 for Gemma4 quantized layer2 transition trace")
	}
	dir := gemma4Path()
	...
	maxAbs, meanAbs := diffStats(cpuFinal[2], cpuOps[opTraceKey{3, "hidden_in"}])
	t.Logf("CPU L2 final_hidden vs L3 hidden_in: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
	maxAbs, meanAbs = diffStats(gpuFinal[2], gpuOps[opTraceKey{3, "hidden_in"}])
	t.Logf("GPU L2 final_hidden vs L3 hidden_in: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
}
