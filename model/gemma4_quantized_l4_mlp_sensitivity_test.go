...
func TestGemma4QuantizedLayer4MLPSensitivity(t *testing.T) {
	if os.Getenv("GEMMA4_TRACE_TEST") == "" {
		t.Skip("set GEMMA4_TRACE_TEST=1 for Gemma4 quantized layer4 MLP sensitivity")
	}
	dir := gemma4Path()
	...
	mlpIn := gpuOps[opTraceKey{4, "mlp_input"}]
	...
	if len(mlpIn) == 0 || len(gpuGate) == 0 || len(gpuUp) == 0 || len(gpuGateAct) == 0 || len(gpuDown) == 0 {
		t.Fatal("missing layer4 mlp traces")
	}
	...
	if layer.GateWm != nil {
		...
		t.Fatal("expected quantized MLX weights for layer4 gate/up")
	}
	bf16Slice(gate)
	bf16Slice(up)
	maxAbs, meanAbs := diffStats(gpuGate, gate)
	t.Logf("L4 cpu(gate from gpu mlp_input) vs gpu gate_pre: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
	maxAbs, meanAbs = diffStats(gpuUp, up)
	t.Logf("L4 cpu(up from gpu mlp_input) vs gpu up: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
	...
	t.Logf("L4 cpu(gate_act from gpu mlp_input) vs gpu gate_act: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)

	down := make([]float32, len(gpuDown))
	if layer.DownWm != nil {
		quant.GemvMLQ(down, gateAct, layer.DownWm)
	} else {
		t.Fatal("expected quantized MLX weights for layer4 down")
	}
	bf16Slice(down)
	maxAbs, meanAbs = diffStats(gpuDown, down)
	t.Logf("L4 cpu(down from gpu mlp_input) vs gpu down: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
}
