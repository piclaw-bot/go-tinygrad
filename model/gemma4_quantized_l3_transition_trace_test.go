...
func TestGemma4QuantizedLayer3TransitionTrace(t *testing.T) {
	if os.Getenv("GEMMA4_TRACE_TEST") == "" {
		t.Skip("set GEMMA4_TRACE_TEST=1 for Gemma4 quantized layer3 transition trace")
	}
	dir := gemma4Path()
	...
	maxAbs, meanAbs := diffStats(cpuFinal[3], cpuOps[opTraceKey{4, "hidden_in"}])
	t.Logf("CPU L3 final_hidden vs L4 hidden_in: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
	maxAbs, meanAbs = diffStats(gpuFinal[3], gpuOps[opTraceKey{4, "hidden_in"}])
	t.Logf("GPU L3 final_hidden vs L4 hidden_in: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
}
