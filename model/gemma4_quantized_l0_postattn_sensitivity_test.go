...
func TestGemma4QuantizedLayer0PostAttnSensitivity(t *testing.T) {
	if os.Getenv("GEMMA4_TRACE_TEST") == "" {
		t.Skip("set GEMMA4_TRACE_TEST=1 for Gemma4 quantized layer0 post-attn sensitivity")
	}
	dir := gemma4Path()
	...
	gpuHiddenIn := gpuOps[opTraceKey{0, "hidden_in"}]
	gpuO := gpuOps[opTraceKey{0, "o"}]
	gpuMLPIn := gpuOps[opTraceKey{0, "mlp_input"}]
	cpuMLPIn := cpuOps[opTraceKey{0, "mlp_input"}]
	if len(gpuHiddenIn) == 0 || len(gpuO) == 0 || len(gpuMLPIn) == 0 || len(cpuMLPIn) == 0 {
		t.Fatal("missing layer0 hidden_in/o/mlp_input traces")
	}

	cpuONorm := append([]float32(nil), gpuO...)
	rmsNormInPlace(cpuONorm, m.Layers[0].PostNorm.Data(), float32(m.Config.RMSNormEps))
	...
	rmsNormInPlace(cpuRecon, m.Layers[0].PreFFNNorm.Data(), float32(m.Config.RMSNormEps))
	bf16Slice(cpuRecon)
	maxAbs, meanAbs := diffStats(gpuMLPIn, cpuRecon)
	t.Logf("L0 cpu(preffn(residual+postnorm(gpuO))) vs gpu mlp_input: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
