...
func TestGemma4QuantizedLayer0TransitionTrace(t *testing.T) {
	if os.Getenv("GEMMA4_TRACE_TEST") == "" {
		t.Skip("set GEMMA4_TRACE_TEST=1 for Gemma4 quantized layer0 transition trace")
	}
	dir := gemma4Path()
	...
	maxAbs, meanAbs := diffStats(cpuFinal[0], cpuOps[opTraceKey{1, "hidden_in"}])
	t.Logf("CPU L0 final_hidden vs L1 hidden_in: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
	maxAbs, meanAbs = diffStats(gpuFinal[0], gpuOps[opTraceKey{1, "hidden_in"}])
	t.Logf("GPU L0 final_hidden vs L1 hidden_in: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
}
