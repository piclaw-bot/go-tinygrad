...
func TestGemma4QuantizedLayer7TransitionTrace(t *testing.T) {
	if os.Getenv("GEMMA4_TRACE_TEST") == "" {
		t.Skip("set GEMMA4_TRACE_TEST=1 for Gemma4 quantized layer7 transition trace")
	}
	dir := gemma4Path()
	...
	maxAbs, meanAbs := diffStats(cpuFinal[7], cpuOps[opTraceKey{8, "hidden_in"}])
	t.Logf("CPU L7 final_hidden vs L8 hidden_in: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
	maxAbs, meanAbs = diffStats(gpuFinal[7], gpuOps[opTraceKey{8, "hidden_in"}])
	t.Logf("GPU L7 final_hidden vs L8 hidden_in: maxAbs=%.6g meanAbs=%.6g", maxAbs, meanAbs)
}
