...
func TestGemma4QuantizedLayer0FinalTailSensitivity(t *testing.T) {
	if os.Getenv("GEMMA4_TRACE_TEST") == "" {
		t.Skip("set GEMMA4_TRACE_TEST=1 for Gemma4 quantized layer0 final-tail sensitivity")
	}
	dir := gemma4Path()
	...
	debugOpHook = func(backend string, step, layer int, op string, vec []float32) {
		if step != traceStep || layer != 0 || op != "hidden_post_pli" {
			return
		}
	...
	cpuHiddenPostPLI := cpuOps[opTraceKey{0, "hidden_post_pli"}]
	gpuHiddenPostPLI := gpuOps[opTraceKey{0, "hidden_post_pli"}]
	cpuHiddenFinal := cpuFinal[0]
	gpuHiddenFinal := gpuFinal[0]
	if len(cpuHiddenPostPLI) == 0 || len(gpuHiddenPostPLI) == 0 || len(cpuHiddenFinal) == 0 || len(gpuHiddenFinal) == 0 {
		t.Fatal("missing layer0 hidden_post_pli/final traces")
	}

	s := m.Layers[0].LayerScalar
	...
	t.Logf("L0 cpu bf16(layerScalar*hidden_post_pli) vs cpu final_hidden: maxAbs=%.6g meanAbs=%.6g scalar=%.6g", maxAbs, meanAbs, s)
	...
	t.Logf("L0 cpu bf16(layerScalar*gpu hidden_post_pli) vs gpu final_hidden: maxAbs=%.6g meanAbs=%.6g scalar=%.6g", maxAbs, meanAbs, s)
}
