...
- NVFP4/FP4 is now an experimental/internal format with metadata parsing, CPU reference decode/GEMV, CUDA upload, and dequant-to-F32 fallback. Placement estimates include NVFP4 packed-weight bytes plus F8 scale and scalar `weight_scale_2` overhead, with embeddings/LM-head estimated as BF16 for inspected NVIDIA checkpoints. Placement plans now report separate NVFP4 resident, GPU-layer, and per-layer expert-set MB so Qwen3 MoE expert-slot sizing can be evaluated separately from dense layer residency.
