...
4. **Native MTP head**
   ...
   - implement K=1 greedy draft step (CPU skeleton now covers preprojection plus a single-token full-attention/MLP MTP block, RoPE when frequency tables are provided, and final MTP norm + main LM-head logits/argmax; history KV integration remains next);
