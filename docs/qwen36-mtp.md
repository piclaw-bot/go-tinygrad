...
4. **Native MTP head**
   ...
   - implement K=1 greedy draft step (CPU skeleton now covers preprojection plus full-attention/MLP MTP block, RoPE when frequency tables are provided, current K/V return, history-aware attention over past+current MTP KV, final MTP norm + main LM-head logits/argmax, and a `QwenNativeMTPDraftState` carrying hidden/K/V/position across draft steps; generation-loop integration remains next);
