...
- [x] define `QwenNativeMTPHead` / layer structs with `fc`, `pre_fc_norm_embedding`, `pre_fc_norm_hidden`, `layers`, and `norm`, plus synthetic shape validation, a tensor-source loader contract, single-file/sharded safetensors-backed tensor sources, and an auto-selecting safetensors source opener for BF16/F32 fixtures;
